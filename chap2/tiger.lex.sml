...
fun yyAction42 (strm, lastMatch : yymatch) = (yystrm := strm;
      (YYBEGIN COMMENT;
                     inc commentLevel;
                     print ("++ (i) commentLevel: " ^ (Int.toString (!commentLevel)) ^ "\n");
                     continue()))
fun yyAction43 (strm, lastMatch : yymatch) = (yystrm := strm;
      (inc commentLevel;
                     print ("++ (c) commentLevel: " ^ (Int.toString (!commentLevel)) ^ "\n");
                     continue()))
fun yyAction44 (strm, lastMatch : yymatch) = (yystrm := strm;
      (dec commentLevel;
                     print ("--     commentLevel: " ^ (Int.toString (!commentLevel)) ^ "\n");
...
fun yyAction45 (strm, lastMatch : yymatch) = let
      ...
      in
        yystrm := strm;
        (print ("->     commentLevel: " ^ (Int.toString (!commentLevel)) ^ "(" ^ yytext ^ ")\n");
                     continue())
      end
fun yyAction46 (strm, lastMatch : yymatch) = let
      ...
      in
        yystrm := strm;
        (Tokens.INT(parseInt(yytext), yypos, yypos + (size yytext)))
      end
...
fun yyAction52 (strm, lastMatch : yymatch) = let
      ...
      in
        yystrm := strm;
        (let val n : int = ord (List.nth (explode yytext, 2))
                                  val c = chr (n - 64)
                              in
...
fun yyAction53 (strm, lastMatch : yymatch) = let
      ...
      in
        yystrm := strm;
        (let val n = valOf (Int.fromString (implode (List.drop (explode yytext, 1))))
                                  val c = chr n
                              in
                                stringBuffer := c::(!stringBuffer)
                              end; continue())
...
fun yyAction57 (strm, lastMatch : yymatch) = let
      ...
      in
        yystrm := strm;
        (stringBuffer := (List.nth (explode yytext, 0))::(!stringBuffer);
                              continue())
      end
       ...
       of NONE => yyAction47(strm, yyNO_MATCH)
        | SOME(inp, strm') =>
            if inp = #"_"
              ...
              then if inp = #"`"
                  then yyAction47(strm, yyNO_MATCH)
                  else yyQ101(strm', yyMATCH(strm, yyAction47, yyNO_MATCH))
            else if inp <= #"z"
              then yyQ59(strm', yyMATCH(strm, yyAction47, yyNO_MATCH))
              else yyAction47(strm, yyNO_MATCH)
      (* end case *))
...
fun yyQ34 (strm, lastMatch : yymatch) = (case (yygetc(strm))
       of NONE => yyAction22(strm, yyNO_MATCH)
        | SOME(inp, strm') =>
            if inp = #"*"
              then yyQ108(strm', yyMATCH(strm, yyAction22, yyNO_MATCH))
...
fun yyQ19 (strm, lastMatch : yymatch) = (case (yygetc(strm))
       of NONE => yyAction45(strm, yyNO_MATCH)
        | SOME(inp, strm') =>
            if inp = #"*"
              then yyQ20(strm', yyMATCH(strm, yyAction45, yyNO_MATCH))
        ...
        | SOME(inp, strm') =>
            if inp = #":"
              ...
              then if inp = #" "
                  ...
                  then if inp = #"!"
                      then yyAction57(strm, yyNO_MATCH)
                      else yyQ7(strm', yyMATCH(strm, yyAction57, yyNO_MATCH))
                else if inp <= #"/"
                  then yyAction57(strm, yyNO_MATCH)
                  else yyQ8(strm', yyMATCH(strm, yyAction57, yyNO_MATCH))
            else if inp = #"f"
              then yyQ6(strm', yyMATCH(strm, yyAction57, yyNO_MATCH))
            else if inp < #"f"
              then if inp = #"]"
